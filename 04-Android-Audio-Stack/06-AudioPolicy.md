...
- [1. 启动与初始化全链路](#1-启动与初始化全链路-initialization)
  - [1.1 生命周期入口](#11-生命周期入口onfirstref)
  - [1.2 配置文件解析](#12-配置文件解析-serialization)
  - [1.3 XML 完整结构解析](#13-audio_policy_configurationxml-完整结构解析)
- [2. AOSP 源码目录结构详解](#2-aosp-源码目录结构详解)
  - [2.1 service/](#21-service--binder-服务层)
  - [2.2 managerdefault/](#22-managerdefault--核心路由管理器)
  - [2.3 engine/interface/](#23-engineinterface--引擎抽象接口)
  - [2.4 enginedefault/](#24-enginedefault--默认引擎-硬编码优先级)
  - [2.5 engineconfigurable/](#25-engineconfigurable--可配置引擎-pfw)
  - [2.6 common/](#26-common--公共数据结构)
  - [2.7 config/](#27-config--xml-解析器)
- [3. 路由决策逻辑深度拆解](#3-路由决策逻辑深度拆解)
  - [3.1 推导三部曲](#31-推导三部曲)
  - [3.2 完整路由决策流程图](#32-完整路由决策流程图)
  - [3.3 设备优先级矩阵](#33-设备优先级矩阵-device-selection-priority)
  - [3.4 Output 选择策略](#34-output-选择策略-flags-匹配)
- [4. AudioPolicyEngine 深度解析](#4-audiopolicyengine-深度解析)
  - [4.1 Engine 架构位置](#41-engine-在整体架构中的位置)
  - [4.2 类继承与调用关系](#42-engine-类继承与调用关系)
  - [4.3 APM ↔ Engine 调用序列](#43-apm--engine-完整调用序列)
  - [4.4 Default Engine 详解](#44-default-engine-详解)
  - [4.5 Configurable Engine (PFW) 详解](#45-configurable-engine-pfw-详解)
  - [4.6 如何选择 Engine](#46-如何选择-engine)
- [5. 录音设备选择详解](#5-录音设备选择详解-recording-device-selection)
  - [5.1 getInputForAttr() 调用链](#51-getinputforattr-完整调用链)
  - [5.2 Source → 设备映射](#52-录音-source--设备--处理-完整映射)
  - [5.2.1 底层决策逻辑 (AOSP 源码)](#521-source--设备选择的底层决策逻辑)
  - [5.3 并发录音冲突处理](#53-并发录音冲突处理)
  - [5.4 同 Source 多 AudioRecord 并发](#54-同-source-多-audiorecord-并发的完整实现机制)
  - [5.5 录音场景实例分析](#55-录音场景实例分析)
- [6. 音量控制体系](#6-音量控制体系-volume-systems)
  - [6.1 音量架构全景](#61-音量架构全景)
  - [6.2 配置文件加载](#62-配置文件加载)
  - [6.3 音量曲线配置示例](#63-音量曲线配置示例)
  - [6.4 音量下发路径](#64-音量下发路径源码跟踪)
- [7. 设备连接/断开处理](#7-设备连接断开处理)
  - [7.1 耳机插入全链路](#71-耳机插入全链路)
  - [7.2 蓝牙 A2DP 特殊处理](#72-蓝牙-a2dp-连接的特殊处理)
- [8. 关键交互：APM 与 AudioFlinger](#8-关键交互apm-与-audioflinger)
  - [8.1 AudioPatch 机制](#81-audiopatch-机制)
  - [8.2 Dynamic Policy Mix](#82-dynamic-policy-mix-registeraudiopolicy)
- [9. AAOS 车载特殊策略](#9-aaos-车载特殊策略)
- [10. 专家调试与 Dump 实战](#10-专家调试与-dump-实战)
  - [10.1 配置文件路径搜索优先级](#101-配置文件路径搜索优先级)
  - [10.2 核心 dumpsys 命令](#102-核心-dumpsys-命令)
  - [10.3 常见排查场景](#103-常见排查场景)
  - [10.4 动态路由调试](#104-动态路由修改调试方法)
