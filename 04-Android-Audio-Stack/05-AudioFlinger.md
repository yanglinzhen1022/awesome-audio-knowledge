...
- [1. AOSP 源码目录结构与核心类](#1-aosp-源码目录结构与核心类)
  - [1.1 源码目录结构](#11-源码目录结构)
  - [1.2 核心类及职责](#12-核心类及职责)
  - [1.3 关键调用关系总览](#13-关键调用关系总览)
- [2. 启动与初始化流程](#2-启动与初始化流程-initialization-sequence)
  - [2.1 进程启动与实例化](#21-进程启动与实例化)
  - [2.2 HAL 加载链路](#22-hal-加载链路-loading-hal)
  - [2.3 输出线程创建时机](#23-输出线程创建时机)
- [3. 线程模型全景图](#3-线程模型全景图-thread-model)
  - [3.1 线程与 Output 对应关系](#31-线程与-output-的对应关系)
  - [3.2 线程优先级与调度](#32-线程优先级与调度)
- [4. Track 生命周期管理](#4-track-生命周期管理)
  - [4.1 Track 的创建](#41-track-的创建)
  - [4.2 Track 状态机](#42-track-状态机)
  - [4.3 Track 类型层次](#43-track-类型层次)
- [5. FastTrack vs NormalTrack 深度对比](#5-fasttrack-vs-normaltrack-深度对比)
  - [5.1 对比表](#51-对比表)
  - [5.2 FastMixer 工作原理](#52-fastmixer-工作原理)
  - [5.3 FastTrack 准入条件](#53-fasttrack-准入条件)
- [6. threadLoop 核心循环源码级剖析](#6-threadloop-核心循环源码级剖析)
  - [6.1 完整处理流程](#61-完整处理流程-threadscpp)
  - [6.2 Buffer 链路详解](#62-buffer-链路详解)
- [7. AudioMixer 混音器深度细节](#7-audiomixer-混音器深度细节)
  - [7.1 Buffer 挂载路径](#71-buffer-挂载路径)
  - [7.2 饱和截断算法](#72-饱和截断算法-saturation)
  - [7.3 混音处理函数选择策略](#73-混音处理函数选择策略)
  - [7.4 增益控制与 Ramp](#74-增益控制与-ramp)
- [8. 重采样器深度解析](#8-重采样器-resampler-深度解析)
  - [8.1 重采样质量等级](#81-重采样质量等级)
  - [8.2 动态多相滤波器](#82-动态多相滤波器-polyphase-resampling)
  - [8.3 重采样导致的延迟](#83-重采样导致的延迟)
- [9. 内存共享机制：Ashmem 与 Proxy](#9-内存共享机制ashmem-与-proxy)
  - [9.1 环形缓冲区结构](#91-环形缓冲区结构)
  - [9.2 Proxy 的原子操作](#92-proxy-的原子操作)
  - [9.3 Buffer 大小计算](#93-buffer-大小计算)
  - [9.4 Underrun 与 Overrun 处理](#94-underrun-与-overrun-处理)
- [10. 专家调试与 Dump 实战分析](#10-专家调试与-dump-实战分析)
  - [10.1 获取 Dump 信息](#101-获取-dump-信息)
  - [10.2 Dump 输出关键字段解读](#102-dump-输出关键字段解读)
  - [10.3 常见问题定位清单](#103-常见问题定位清单)
  - [10.4 实时监控脚本](#104-实时监控脚本)
